...
def get_image_info(image):
    ...
    if depth == 1:
        # Single-channel
        info["statistics"]["min"] = np.min(image).item()
        info["statistics"]["max"] = np.max(image).item()
        info["statistics"]["mean"] = np.mean(image).item()
        info["statistics"]["std_dev"] = np.std(image).item()
        info["statistics"]["median"] = np.median(image).item()
        ...
        for idx, ch in enumerate(channels):
            ch_stats = {
                "min": np.min(ch).item(),
                "max": np.max(ch).item(),
                "mean": np.mean(ch).item(),
                "std_dev": np.std(ch).item(),
                "median": np.median(ch).item(),
            }
